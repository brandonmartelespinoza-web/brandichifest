...
const OUT = process.argv[2] || '/home/user/work/audit1';
const b = await chromium.launch(); const ctx = await b.newContext({ viewport: { width: 1280, height: 900 } });
await ctx.route(/fonts\.(googleapis|gstatic)\.com/, r => r.fulfill({ status: 200, contentType: 'text/css', body: '' }));
const p = await ctx.newPage(); await p.goto('http://127.0.0.1:8719/GUIA-SEM19.html', { waitUntil: 'load' }); await p.addStyleTag({ content: 'nav{position:static!important}' });
const rep = [];
for (const t of ['par','pul','med','mec','esp','gas','vqr','tim','org']) {
